...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 /*elapsed*/, bool /*minimal*/)
{
    ...
    SetNextCheckDelay(sPlayerbotAIConfig.randomBotUpdateInterval * IN_MILLISECONDS);
    ...
    // While filling the population, devote the entire batch to logins. Splitting
    // it between updates and logins makes RandomBotsPerInterval an inaccurate
    // admission limit and amplifies startup database pressure.
    uint32 updateBots = onlineBotCount < maxAllowedBotCount ? 0 :
        sPlayerbotAIConfig.randomBotsPerInterval * onlineBotFocus / 100;
    uint32 maxNewBots =
...
void RandomPlayerbotMgr::AssignAccountTypes()
{
    ...
    // This table may contain stale rows for human accounts after an older import.
    // Keep it strictly scoped to the configured bot-account prefix so those rows
    // cannot affect pool sizing or ever become eligible for bot assignment.
    PlayerbotsDatabase.Execute(
        "DELETE pat FROM playerbots_account_type pat "
        "LEFT JOIN {}.account a ON a.id = pat.account_id "
        "WHERE a.id IS NULL OR a.username NOT LIKE '{}%%'",
        LoginDatabase.GetConnectionInfo()->database,
        sPlayerbotAIConfig.randomBotAccountPrefix.c_str());
