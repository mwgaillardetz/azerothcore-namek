-- Allow deferred Guild greetings for real-player logins.

SET @has_guild_login_greeting = (
  SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'llm_chatter_events'
    AND COLUMN_NAME = 'event_type'
    AND COLUMN_TYPE LIKE '%guild_login_greeting%'
);

SET @sql = IF(
  @has_guild_login_greeting = 0,
  CONCAT(
    ...
    "'guild_idle_chatter','guild_player_message',",
    "'guild_login_greeting') NOT NULL"
  ),
  'SELECT 1'
);
