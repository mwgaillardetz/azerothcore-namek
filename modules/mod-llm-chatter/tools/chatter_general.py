...
def _pick_random_traits(seed=''):
    """Pick 3 traits for a bot.

    Deterministic per-bot when a seed (name/guid) is
    given, so the same bot keeps a consistent
    personality across calls. Random when no seed.
    """
    if seed:
        rng = random.Random(
            f"traits:{seed}".lower()
...
def _get_bot_info(db, bot_guid):
    """Fetch character data plus the bot's persistent identity."""
    cursor = db.cursor(dictionary=True)
    cursor.execute("""
        SELECT c.name, c.class, c.race, c.level, c.gender,
               i.trait1, i.trait2, i.trait3, i.tone, i.backstory
        FROM characters c
...
def _select_primary_bot(
    ...
    return {
        ...
        'bot1_traits': (
            [trait for trait in (
                bot1_info.get('trait1'),
                bot1_info.get('trait2'),
                bot1_info.get('trait3'),
            ) if trait]
            or _pick_random_traits(str(bot1_guid))
        ),
        'bot1_tone': bot1_info.get('tone') or '',
        'bot1_backstory': bot1_info.get('backstory') or '',
...
def _build_general_response_prompt(
    ...
    if not is_rp and backstory:
        prompt += (
            "Your private real-life background (use it as subtle perspective; "
            f"never recite it): {backstory}\n"
        )
    if twist:
    ...
    gameplay_guidance = retrieve_gameplay_guidance(
        player_message, map_id=map_id,
    )
    if gameplay_guidance:
        prompt += f"{gameplay_guidance}\n\n"
    prompt += (
        ...
        f"- If the player asks for gameplay help, prioritize a direct, useful "
        f"answer for WoW 3.3.5a. Give concrete level ranges, zones, routes, "
        f"entrances, roles, or mechanics when relevant. Never mix in later "
        f"expansion systems; say when you are unsure instead of guessing\n"
        f"{address_hint}"
...
def process_general_player_msg_event(
    ...
    try:
        ...
        prompt1 = _build_general_response_prompt(
            ...
            stored_tone=primary.get('bot1_tone', ''),
            backstory=primary.get('bot1_backstory', ''),
        )
