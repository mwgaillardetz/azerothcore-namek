...
def _call_fallback_provider(
    provider, model, sys_msg, user_msg, config,
    max_tokens, temperature, label,
):
    """Call one configured OpenAI-compatible fallback provider."""
    import openai
    ...
    response = openai.OpenAI(**kwargs).chat.completions.create(
        **request
    )
    return _extract_chat_content(response, label), sent_user_msg
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        result = None
        fallback_keys = (
            ('LLMChatter.Fallback.Provider',
             'LLMChatter.Fallback.Model'),
            ('LLMChatter.Fallback2.Provider',
             'LLMChatter.Fallback2.Model'),
        )
        for provider_key, model_key in fallback_keys:
            ...
            fallback_model = config.get(model_key, '').strip()
            if not fallback_model:
                continue

            try:
                logger.warning(
                    "Falling back to %s model %s (%s)",
                    fallback_provider, fallback_model, label,
                )
                result, sent_user_msg = _call_fallback_provider(
                    fallback_provider, fallback_model,
                    sys_msg, user_msg, config, max_tokens,
                    temperature, label,
                )
                logged_provider = fallback_provider
                logged_model = fallback_model
                if result:
                    break
            except Exception as fallback_exc:
                logger.error(
                    "Fallback %s call failed (%s): %s",
                    fallback_provider, label, fallback_exc,
                )
    finally:
        duration_ms = int(
        ...
        try:
            ...
            log_request(
                ...
                logged_model, logged_provider, duration_ms,
                metadata=metadata,
