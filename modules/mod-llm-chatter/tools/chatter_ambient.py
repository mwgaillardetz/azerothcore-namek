...
def _pick_ambient_topic(
    bot: dict, topic_pool: list
) -> str:
    """Pick an ambient topic biased toward the bot's
    personality archetype (news/sports lean).
    """
    return pick_personality_topic(
        bot.get('name', ''),
        bot.get('guid'),
        base_pool=topic_pool,
...
def process_statement(
    ...
    if msg_type == "plain":
        ...
        topic = _pick_ambient_topic(bot, topic_pool)
        chosen_topic = topic
...
def process_conversation(
    ...
    if msg_type == "plain":
        ...
        topic = _pick_ambient_topic(bots[0], topic_pool)
        chosen_topic = topic
