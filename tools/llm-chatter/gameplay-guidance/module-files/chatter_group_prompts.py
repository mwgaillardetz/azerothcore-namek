...
def build_bot_greeting_prompt(
    ...
    if not is_rp and bot.get('backstory'):
        prompt += (
            "Your private real-life background (use it as subtle perspective; "
            f"never recite it): {bot['backstory']}\n"
        )
    if twist:
...
def build_nearby_object_conversation_prompt(
    ...
    if not is_rp:
        backgrounds = [
            f"  {bot['name']}: {bot.get('backstory')}"
            for bot in bots if bot.get('backstory')
        ]
        if backgrounds:
            parts.append(
                "Private real-life backgrounds (draw from them subtly; never "
                "recite biographies):\n" + "\n".join(backgrounds)
            )
...
def build_player_msg_conversation_prompt(
    ...
    if gameplay_guidance:
        parts.append(gameplay_guidance)
    ...
    parts.append(
        ...
        "- If the player asks for gameplay help, the first speaker gives a "
        "direct, useful WoW 3.3.5a answer with concrete level ranges, zones, "
        "routes, entrances, roles, or mechanics when relevant; never mix in "
        "later-expansion systems and admit uncertainty rather than guessing\n"
