...
def get_chatter_mode(config: dict) -> str:
    """Resolve normal, roleplay, or a per-conversation hybrid mode."""
    if not config:
        return 'normal'
    mode = config.get('LLMChatter.ChatterMode', 'normal').lower()
    if mode == 'hybrid':
        try:
            human_pct = int(config.get(
                'LLMChatter.HumanConversationChance', 85
            ))
        except (TypeError, ValueError):
            human_pct = 85
        human_pct = max(0, min(human_pct, 100))
        return (
            'normal'
            if random.random() < human_pct / 100.0
            else 'roleplay'
        )
...
def find_addressed_bot(
    ...
    try:
        result = quick_llm_analyze(
            client, config, prompt, max_tokens=500,
...
# =============================================================================
# PERSONALITY ARCHETYPES & TOPIC BIAS
# =============================================================================

def get_bot_archetype(
    name: str,
    guid: Optional[int] = None,
    seed_str: str = '',
) -> Dict[str, Any]:
    """Deterministically pick a personality archetype
    for a speaker so their voice stays consistent.

    Uses a stable hash of the speaker name/guid so the
    same bot always gets the same archetype across
    restarts. Falls back to the first archetype if the
    pool is empty.
    """
    if not PERSONALITY_ARCHETYPES:
        return {}
    seed = f"{name}|{guid}|{seed_str}".lower()
    h = 0
    for ch in seed:
        h = (h * 31 + ord(ch)) & 0x7FFFFFFF
    return PERSONALITY_ARCHETYPES[h % len(PERSONALITY_ARCHETYPES)]


def pick_personality_topic(
    name: str,
    guid: Optional[int] = None,
    base_pool: Optional[List[str]] = None,
    news_pool: Optional[List[str]] = None,
    sports_pool: Optional[List[str]] = None,
    lean_chance: float = 0.55,
) -> str:
    """Pick a chat topic, occasionally biasing toward
    the speaker's personality archetype.

    lean_chance controls how often the archetype's
    topic lean wins over the generic pool. Pass the
    chat's base/news/sports pools explicitly so this
    stays import-light.
    """
    archetype = get_bot_archetype(name, guid)
    leans = archetype.get('topic_lean', [])

    if (
        leans
        and random.random() < lean_chance
    ):
        # Pick a lean that has a dedicated pool
        dedicated = []
        for lean in leans:
            if lean == 'news' and news_pool:
                dedicated.extend(news_pool)
            elif lean == 'sports' and sports_pool:
                dedicated.extend(sports_pool)
        if dedicated:
            return random.choice(dedicated)

    # Fall through to the base pool
    if base_pool:
