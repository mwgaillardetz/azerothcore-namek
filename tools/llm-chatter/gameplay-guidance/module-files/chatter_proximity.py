...
def _single_prompt(
    ...
    # Deterministic personality archetype fallback so
    # every speaker has a distinct, consistent voice
    # even without DB traits.
    archetype = get_bot_archetype(
        speaker.get('name', ''),
        int(speaker.get('bot_guid', 0) or 0)
        or speaker.get('npc_spawn_id'),
        seed_str=speaker_desc,
    )
    if not speaker_traits and archetype:
        speaker_traits = archetype.get('traits', [])
    if not speaker_tone and archetype:
        speaker_tone = archetype.get('tone', '')
    ...
    opening = (
        ...
        "or class unless the immediate topic genuinely calls for it."
        if human_style else
        "You write extremely short, immersive World of Warcraft in-world "
        "/say lines."
    )
    lines = [
        opening,
        ...
        "and low-stakes.",
        (
            "Use candid human conversation and everyday perspective. No AI "
            "talk or markdown."
            if human_style else
            "Keep it lore-friendly. No modern memes, no AI talk, no markdown."
        ),
        "",
    ...
    if speaker_backstory and config:
        ...
        if human_style:
            prox_chance = max(prox_chance, 0.70)
...
def _conversation_prompt(
    ...
    if participants:
        topic = pick_personality_topic(
            ...
            or participants[0].get('npc_spawn_id'),
            base_pool=PROXIMITY_CHAT_TOPICS,
            news_pool=NEWS_CHAT_TOPICS,
            sports_pool=SPORTS_CHAT_TOPICS,
        )
    zone_name = extra.get('zone_name', 'the area')
    subzone_name = extra.get('subzone_name', '')
    max_lines = max(
    ...
    if config:
        ...
        if human_style:
            _bs_chance = max(_bs_chance, 0.70)
    ...
    for speaker in participants:
        ...
        archetype = get_bot_archetype(
            ...
            int(speaker.get('bot_guid', 0) or 0)
            or speaker.get('npc_spawn_id'),
            seed_str=_describe_speaker(db, speaker),
        )
        if not speaker.get('is_npc'):
            profile = _query_bot_traits(
                db,
            ...
            if not traits and archetype:
                traits = archetype.get('traits', [])
            if not tone and archetype:
    ...
    opening = (
        "Write a short overheard /say exchange between ordinary real people "
        "playing World of Warcraft. Let their jobs, families, hobbies, moods, "
        "and personal histories shape the chat. Do not make them perform "
        "their fantasy races or classes constantly."
        if human_style else
        "You write short World of Warcraft ambient overheard /say conversations."
    )
    lines = [
        opening,
...
def _generate_single_line(
    ...
    *,
    ...
    prompt = _single_prompt(
        ...
        extra,
        speaker,
        topic or pick_personality_topic(
            speaker.get('name', ''),
            int(speaker.get('bot_guid', 0) or 0)
            or speaker.get('npc_spawn_id'),
            base_pool=PROXIMITY_CHAT_TOPICS,
...
def _player_say_single_prompt(
    ...
    opening = (
        "You are an ordinary real person playing World of Warcraft. Write "
        "natural /say chat like a human player, not a fantasy-race "
        "performance. Your character can be a troll, dwarf, or anything "
        "else without talking like that race constantly. Modern everyday "
        "life, work, family, hobbies, and casual player language are welcome."
        if human_style else
        "You write extremely short, immersive World of Warcraft in-world "
        "say lines."
    )
    lines = [
        opening,
        "Normally use 8-15 words. If the player asks a gameplay question, "
        "use up to 35 words so the answer can be useful.",
        "Gameplay advice must target WoW 3.3.5a: give concrete level ranges, "
        "zones, routes, entrances, roles, or mechanics when relevant; never "
        "mix in later-expansion systems, and admit uncertainty rather than "
        "guessing.",
        (
            "Sound candid and conversational. No AI talk or markdown."
            if human_style else
            "Keep it lore-friendly. No modern memes, no AI talk, no markdown."
        ),
        "",
    ...
    if human_style:
        ...
        if profile.get('tone'):
            lines.append(f"Your natural tone: {profile['tone']}")
        if profile.get('backstory'):
            lines.append(
                "Your private background (use as subtle perspective; never "
                f"recite it): {profile['backstory']}"
            )
    if subzone_name:
        lines.append(f"Subzone: {subzone_name}")
    ...
    guidance = retrieve_gameplay_guidance(
        player_message,
...
def _player_say_conversation_prompt(
    ...
    if human_style:
        personalized = []
        for speaker in participants:
            desc = _describe_speaker(db, speaker)
            profile = _query_bot_traits(
                db, int(speaker.get('bot_guid', 0) or 0)
            )
            details = list(profile.get('traits', []))
            if profile.get('tone'):
                details.append(f"tone: {profile['tone']}")
            if profile.get('backstory'):
                details.append(
                    "private background to draw from subtly: "
                    + profile['backstory']
                )
            personalized.append(
                f"- {desc}" + ("; " + "; ".join(details) if details else "")
            )
        roster = "\n".join(personalized)
    opening = (
        "Write a short /say exchange between ordinary real people playing "
        "World of Warcraft. They are human players first: use natural casual "
        "conversation, modern daily-life perspective, and personal opinions. "
        "Do not make them perform their character races or classes constantly."
        if human_style else
    ...
    lines = [
        ...
        "Keep the exchange brief and immersive.",
        "If the player asks for gameplay help, the first speaker must give a "
        "direct answer for WoW 3.3.5a with concrete details when known. Never "
        "mix in later-expansion systems; admit uncertainty rather than guess.",
        "",
        f"Zone: {zone_name}",
    ]
    if subzone_name:
